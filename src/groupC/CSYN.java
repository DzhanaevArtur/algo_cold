...
public class CSYN extends LN {
    ...
    // Настройки
    private ASG vNom = new ASG(0.0F); //  Номинальное вторичное напряжение
    private ASG hzNom = new ASG(0.0F); //  Номинальная частота
    private ASG vAdpFact = new ASG(0.0F); //  Коэффициент адаптации U1/U2
    private ASG adpAngDeg = new ASG(0.0F); //  Угол адаптации
    private ING dlTmms = new ING(0); //  Время контроля для распараллеливания
    private ASG mltCmd = new ASG(0.0F); //  Генерация нескольких команд
    private ASG difVNg = new ASG(0.0F); //  Разностное напряжение отрицательный
    private ASG difVPs = new ASG(0.0F); //  Разностное напряжение положительное
    private ASG difHzNg = new ASG(0.0F); //  Разностная частота отрицательный
    private ASG difHzPs = new ASG(0.0F); //  Разностная частота положительная
    private ASG difAngNg = new ASG(0.0F); //  Разностный фазовый угол отрицательный
    private ASG difAngPs = new ASG(0.0F); //  Разностный фазовый угол положительный
    private ASG minVSyn = new ASG(0.0F); //  Минимальное напряжение для синхронизации в режиме реального времени
    private ASG maxVSyn = new ASG(0.0F); //  Максимальное напряжение для синхронизации в режиме реального времени
    private ASG detSyn = new ASG(0.0F); //  Обнаружение синхронности (Δf)
    private ENG livDeaMod = new ENG(); //  Режим живых мертвецов
    private ASG deaLinVal = new ASG(0.0F); //  Значение мертвой линии
    private ASG livLinVal = new ASG(0.0F); //  Значение линии в реальном времени
    private ASG deaBusVal = new ASG(0.0F); //  Значение мертвой шины
    private ASG livBusVal = new ASG(0.0F); //  Значение шины в реальном времени
    private SPG vAdj = new SPG(); //  Включение/Выключение датчика соответствия напряжений
    private ASG vChr = new ASG(0.0F); //  Характеристика регулировки напряжения
    private ING vInvTmms = new ING(0); //  Интервал импульсов регулировки напряжения
    private ING minVTmms = new ING(0); //  Минимальное время импульса регулировки напряжения
    private ING maxVTmms = new ING(0); //  Максимальное время импульса регулировки напряжения
    private SPG hzAdj = new SPG(); //  Включение/Выключение датчика частоты
    private ASG hzChr = new ASG(0.0F); //  Характеристика регулировки частоты
    private ING hzIntvTmms = new ING(0); //  Интервал импульсов регулировки частоты
    private ING minHzTmms = new ING(0); //  Минимальное время импульса регулировки частоты
    private ING maxHzTmms = new ING(0); //  Максимальное время импульса регулировки частоты
    private ASG hzTgtVal = new ASG(0.0F); //  Целевое значение преобразователя частоты
    private SPG kckPls = new SPG(); //  Включение/выключение импульса кикера
    private ING dlSynTmms = new ING(0); //  Задержка процесса синхронизации после сигнала запуска
    private ING totTmms = new ING(0); //  Общее время процесса синхронизации
